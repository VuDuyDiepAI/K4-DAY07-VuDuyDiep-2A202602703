...
class EmbeddingStore:
    ...
    def add_documents(self, docs: list[Document]) -> None:
        ...
        embeddings = embed_many([doc.content for doc in docs]) if callable(embed_many) else None
        ...
        for offset, doc in enumerate(docs):
            embedding = embeddings[offset] if embeddings is not None else None
            record = self._make_record(doc, embedding=embedding)
            # Documents produced from chunks may share a parent doc_id; storage IDs
            # must nevertheless remain unique, especially for Chroma collections.
            record["id"] = f"{doc.id}:{self._next_index + offset}"
